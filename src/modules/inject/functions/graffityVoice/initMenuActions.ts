...
function initMenuActions() {
  document.arrive(".VKCOMMessenger__reforgedModalRoot > .MEConfig > .MEPopper > div > .DropdownReforged__contentWrapper > .ActionsMenu:not(#member_actions)", { existing: true }, function (e) {
    ...
    createStyle("MEPopperStyle", `.VKCOMMessenger__reforgedModalRoot > .MEConfig > .MEPopper:not(:has(#member_actions)){margin-top:${isGim ? "-48px" : "-72px"}!important;}`);
    ...
    if (!document.querySelector(".GraffitiMenuPopper") && !isGim) {
    ...
    if (!document.querySelector(".StickerMenuPopper") && !isGim) {
