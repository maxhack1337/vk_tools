...
/*
 * Либо ВК сами что то сломают
 * Либо селектор
 * Но функция важная, нужно поддерживать
 */

const pollResults = () => {
  if (localStorage.getItem("pollResultsValue") === "true") {
    document.arrive("[class^='vkitPrimaryAttachmentPoll__voting']", { existing: true }, function (e) {
      ...
      var polls = document.querySelectorAll('[class^="vkitPollOptions__votingOptions"]');
      for (var poll of polls) {
        poll.classList.add("VKEnhancer-module__votingOptionsVoted");
      }
      var percentageElements = document.querySelectorAll('[class^="vkitPollOptions__votingOptionRight"]');
      percentageElements.forEach(function (element) {
        if (element.textContent) {
          var percentage = parseFloat(element.textContent.replace("%", ""));
          var parentElement = element.closest('[class^="vkitPollOptions__votingOptionWrapper"]');
          var fillElement = parentElement?.querySelector('[class^="vkitPollOptions__votingOptionFill"]');
          if (fillElement instanceof HTMLElement) fillElement.style.width = percentage + "%";
        }
      });
    });
    document.arrive("[class^='vkitModalBody__container'] > [class^='vkitPollOptions__votingOptionsWrapper']", { existing: true }, function (e) {
      let styleElement = fromId("PollResultsShow");
      if (!styleElement) {
        styleElement = document.createElement("style");
        styleElement.id = "PollResultsShow";
    ...
    document.arrive(".media_voting", { existing: true }, async function (e) {
      if (e instanceof HTMLElement) {
        ...
        styleElement.innerHTML = ".media_voting_multiple .media_voting_option_text::after{right:52px!important;}";
      }
    });

    document.arrive(".AttachPoll__answers", { existing: true }, async function (e) {
