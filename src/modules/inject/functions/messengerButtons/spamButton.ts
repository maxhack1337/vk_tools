/*
 * Делал Паша, функция полезная, но она на kphp
 * Подумай, мб через API можно получать и сделать бокс по красоте
 */

const spamButton = () => {
  document.arrive(".BurgerMenu__actionsMenu", { existing: true }, function (e) {
    let burgerim = document.querySelector(".BurgerMenu__actionsMenu > div > div > div");
