  ...
  try {
    ...
    if (!repost?.querySelector(".vkEnhancerPostDate")) {
      ...
      if (isCase === "" || isCase === "post") {
        ...
        postDate.innerHTML =
          `
				<a class="vkEnhancerPostDate published_by_date" href="/` +
          postType +
          repostID +
          `" onclick="event.preventDefault(); event.stopPropagation(); window.showVideo('` +
          repostID +
          `','0',{autoplay: 1, queue: 0, listId: '', playlistId: ''}, this);">
					` +
          getFormattedPostDate(repostData.items[0].date) +
          `
				</a>
        ...
        postDate.innerHTML =
          ...
          postType +
          repostID +
          `" onclick="event.preventDefault(); event.stopPropagation(); window.showPhoto('` +
          repostID +
          `');">
					` +
          getFormattedPostDate(repostData.items[0].date) +
