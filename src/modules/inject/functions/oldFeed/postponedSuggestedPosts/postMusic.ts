...
const postMusic = () => {
  let selectorsMusic = [
    `.postponed.Post--redesignV3 [class^="OnMediaAttachmentOverlay__attachment"]`,
    `.suggest.Post--redesignV3 [class^="OnMediaAttachmentOverlay__attachment"]`,
    `.postponed.Post--redesignV3 [class^="vkitMusicOverlayAttachment__root"]`,
    `.suggest.Post--redesignV3 [class^="vkitMusicOverlayAttachment__root"]`,
  ];
  document.arrive(selectorsMusic.join(", "), { existing: true }, async function (docus) {
    let closestCheck = docus.closest(".vkEnhancerPostponedPostMusic");
    if (!closestCheck) {
      let dataAttachments;
      let postAppendClass = docus.closest(".Post--redesignV3");
      postAppendClass?.classList.add("vkEnhancerPostponedPostMusic");
      try {
