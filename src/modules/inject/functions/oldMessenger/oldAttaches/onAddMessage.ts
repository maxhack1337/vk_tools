...
const onAddMessage = async (message: HTMLElement) => {
	...
	if (gift || attaches || fwd || story || vidMsg) {
		...
		let findCurrMessagesFromPeerDiff =
			diff.length > 0 ? diff.find((item: any) => item.peer_id === memoizedPeer)?.messages : null;
		let findCurrMessagesFromPeerHistory =
			history.length > 0 ? history.filter((item: any) => item.peer_id === memoizedPeer) : null;
		let findCurrMessagesFromPeerSent =
			sentMessages.length > 0 ? sentMessages.filter((item: any) => item.peer_id === memoizedPeer) : null;
