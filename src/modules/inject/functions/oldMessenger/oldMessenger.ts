	...
	if (getLocalValue("oldMessengerDes") === true) {
		...
		document.arrive(".ConvoMessageInfoWithoutBubbles", { existing: true }, function (e) {
			...
			let messageAuthorLink = time
				.closest(".ConvoMessageWithoutBubble__wrapper")
				?.querySelector(".ConvoMessageHeader__sentFromInfo");
		...
		document.arrive(".BurgerMenu__actionsMenu > div > div > div", { existing: true }, function (e) {
			...
			changeDesign.classList.add(
				"ActionsMenuAction",
				"ActionsMenuAction--secondary",
				"ActionsMenuAction--size-regular"
			);
		...
		document.arrive('#archive[class^="vkitRightMenuItem"]', { existing: true }, async function (e) {
			...
			if (div?.textContent) {
				try {
					div.textContent = IMLang.keys.me_convo_list_important_messages;
				} catch (e) {
					div.textContent = "Важные сообщения";
				}
			}
