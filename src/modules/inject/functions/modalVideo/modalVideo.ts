...
const modalVideo = () => {
  if (getLocalValue("videoModal")) {
    ...
    document.arrive(`[class^="vkitVideoCardLayout__card"]:has([class^="vkitVideoCardThumb__thumb"][href^="/video"]):not([class*="VideoCardLayout__cardHorizontal"])`, { existing: true }, async function (e) {
      let videoLink;
      try {
        videoLink = e?.querySelector('[class^="vkitVideoCardThumb__thumb"][href^="/video"]')?.getAttribute("href")?.substring(6);
      } catch (error) {}
