    ...
    <a href="/albums${userIDHereWeGoAgain}" data-allow-link-onclick-web="1" style="padding: 0 8px;" class="Header-module__tappable--mabke ProfileGroupHeader vkuiTappable vkuiInternalTappable vkuiTappable--hasActive vkui-focus-visible">
      <div class="vkuiHeader vkuiHeader--mode-primary vkuiHeader--pi Header-module__header--a6Idw Header-module__headerPrimary--mmJ1C" role="heading" aria-level="2">
        <div class="vkuiHeader__main">
          ...
          ${getPhotoAlbumLang(vk.lang)}</div></div></span><span class="vkuiTypography vkuiTypography--normalize vkuiTypography--weight-2 vkuiHeader__indicator vkuiFootnote">${albumsGetter.count}</span></div>
        </div>
      </div>
    </a>
  ...
  <div class="vkuiSpacing" style="height: 12px; padding: 6px 0px;"></div>`;
      if (albumsGetter.count > 0 && !document.querySelector(".ProfileAlbumsEnhancer")) {
        ...
        albumsGetter.items.slice(0, 2).forEach(async (item: { owner_id: any; thumb_id: number; id: any; size: any; title: any }) => {
          ...
          if (item.thumb_id !== 0) {
            albumElement.innerHTML = `<a href="/album${item.owner_id}_${item.id}" data-href="album${item.owner_id}_${item.id}?rev=1" onclick="return showPhoto('${item.owner_id}_${item.thumb_id}', 'album${item.owner_id}_${
              item.id
            }/rev', {}, event); return nav.go(this, event)" class="img_link  photos_album_w_description vkenh">
      ...
      try {
        ...
        if (audioResponse.count > 0 && !document.querySelector(".vkEnAudioRow")) {
          ...
          newAudioElement.innerHTML = ` <div class="vkuiGroup__header">
              ...
              "vkapps_scope_audio"
            )}</div></div></span><span class="vkuiTypography vkuiTypography--normalize vkuiTypography--weight-2 vkuiHeader__indicator vkuiFootnote">${audioResponse.count}</span></div>
						</div>
					</div>
					</a>
					<div class="vkuiSpacing" style="height: 4px; padding: 2px 0px;"></div>
          ...
          audioResponse.items.slice(0, 6).forEach(
            async (audioItem: {
              title: {
              ...
              const audioData = `[${audioItem.id},${audioItem.owner_id},"","${escapeHtml(titleAud)}","${escapeHtml(artistAud)}",157,0,0,"",0,34,"module:${audioItem.owner_id}","[]","62efa83eaf32d46ab7//e3727249bcd60c36ee///bca050eaeb2ae61a22/","",{"duration": ${
                audioItem.ads.duration
              },"content_id": "${audioItem.owner_id}_${audioItem.id}","puid22": ${audioItem.ads.puid22},"account_age_type": ${audioItem.ads.account_age_type},"_SITEID": 276,"vk_id": ${
                vk.id
              },"ver": 251116},"","","",false,"9c91d4359kPPl-j5wiDD-N-q4xNYySV8d1i8YjJXvg6StjuAn436s3dh-U5Vim743w",0,0,true,"${audioItem.access_key}",false,"",false]`;
              audioElement.innerHTML = `<div tabindex="0" class="audio_row audio_row_with_cover _audio_row _audio_row_${audioItem.owner_id}_${audioItem.id} audio_can_add audio_lpb audio_row2 audio_row_playable audio_new_lyrics" data-full-id="${audioItem.owner_id}_${
                audioItem.id
              }" onclick="return getAudioPlayer().toggleAudio(this, event)" onmouseover="window.AudioUtils &amp;&amp; window.AudioUtils.onRowOver(this, event, false, '', '${audioItem.access_key}')" onmouseleave="window.AudioUtils &amp;&amp; window.AudioUtils.onRowLeave(this, event)">
    ...
    <div class="audio_row__inner">
      ...
      <div class="audio_row__performer_title">
        ...
        <div class="audio_row__title _audio_row__title" onmouseover="setTitle(this)">
          <a href="" class="audio_row__title_inner _audio_row__title_inner" data-testid="audio_row_title">${escapeHtml(audioItem.title)}</a>
