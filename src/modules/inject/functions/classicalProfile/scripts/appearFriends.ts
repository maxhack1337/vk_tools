...
import friendsBlock from "./skeleton/friendsBlock";
  ...
  document.arrive("#profile_redesigned", { existing: true }, async function (e) {
    ...
    `;
      ...
      if (vk.id === userdata.id) {
        const onlineFriends = friends.items.filter((friend: { online: number }) => friend.online === 1);
      ...
      let commonFriends = friends.items.filter((friend: { id: string }) => myFriends.includes(friend.id));
