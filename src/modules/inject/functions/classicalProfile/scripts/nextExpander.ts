import { escapeHtml,escapeUrl } from "../../../escapeHtml";
      ...
      const nextExpander = async(userData: { universities: any; schools: any; military: string | any[]; personal: { alcohol?: any; life_main?: any; people_main?: any; smoking?: any; inspired_by?: any; religion?: any; political?: any; langs_full?: any}; activities: string; interests: string; music: string; movies: string; tv: string; books: string; games: string; quotes: string; about: string; }, moreItemsLoaded: { appendChild: (arg0: HTMLDivElement) => void; }, educationInner: any) => {
        ...
        if (education) {
          education.forEach((edu: { name: string | null; country: any; city: any; id: any; graduation: number; faculty_name: string | null; university: any; faculty: any; chair_name: string | null; chair: any; education_form: string | null; education_form_id: any; education_status: string | null; education_status_id: any; }) => {
        ...
        if (schools) {
          schools.forEach((school: { name: string | null; country: any; city: any; id: any; year_from: any; year_to: any; class: any; year_graduated: any; speciality: string | null; }) => {
            ...
            if (school.year_from && school.year_to) {
              ...
              yearFromDiv.textContent = Array.isArray(yearFromString) ? yearFromString[0].replace(
                "%s",
                `${school.year_from}`
              ) : yearFromString?.replace(
                "%s",
                `${school.year_from}`
              ) || `с ${school.year_from} г.`;
              if (school.city) {
                vkApi
                  .api("database.getCitiesById", { city_ids: school.city })
                  .then((cidSchool: { title: string; }[]) => {
          ...
          if (military) {
            military.forEach((voin: { unit: string | null; country_id: any; unit_id: any; from: any; until: any; }) => {
              ...
              if (voin.from && voin.until) {
                ...
                voinFromDiv.textContent = Array.isArray(voinFromString) ? voinFromString[0].replace(
                  "%s",
                  `${voin.from}`
                ) : voinFromString?.replace(
                  "%s",
                  `${voin.from}`
                ) || `с ${voin.from} г.`;
                ...
                voinUntilDiv.textContent = Array.isArray(voinUntilString) ? voinUntilString[0].replace(
                  "%s",
                  `${voin.until}`
                ) : voinUntilString?.replace(
                  "%s",
                  `${voin.until}`
                ) || `до ${voin.until} г.`;
