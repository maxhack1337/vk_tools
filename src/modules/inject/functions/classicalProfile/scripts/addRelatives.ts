...
const addRelatives = async (userData: { relatives: any }, moreItemsLoaded: { appendChild: (arg0: HTMLDivElement) => void }, cachedRelativesInfo: any) => {
    ...
    Object.keys(relativesByType).forEach(async (type) => {
      ...
      relativesOfType.forEach(async (relative) => {
        ...
        if (relative.name) {
          ...
          let relativeData = cachedRelativesInfo[relative.id];
          if (relativeData) {
            let name = relativeData;
            let id = relative.id.toString();
            if (id.startsWith("-")) {
              id = id.substring(1);
