...
const appearStarts = async (userData: {
	...
	if (birthday) {
		...
		if (parts.length === 3) {
			...
			ageAndZodiac = `${getLangTime(
				age,
				getLang?.("global_years_accusative", "raw") || ["", "%s год", "%s года", "%s лет"]
	...
	if (occupation && occupation.type === "university") {
		...
		if (occupation.graduate_year) {
			graduate = `<a href="/search/people?education_city_id=${occupation.city_id}&university=${comid}&university_year=${
