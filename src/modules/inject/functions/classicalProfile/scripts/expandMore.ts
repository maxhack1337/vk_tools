...
const expandMore = async (
  ...
  userData: {
    ...
    schools: any;
    military: string | any[];
    personal: { langs_full?: any; alcohol?: any; life_main?: any; people_main?: any; smoking?: any; inspired_by?: any; religion?: any; political?: any };
    activities: string;
    interests: string;
    music: string;
    movies: string;
    ...
    if (!target?.closest(".vkEnhancerMoreItems")) {
      if (profileMoreLabel?.style.display !== "none") {
        ...
        if ((userData.home_town && userData.home_town !== "") || (userData.personal && userData.personal.langs_full) || (userData.relatives && userData.relatives.length > 0)) {
          ...
          try {
            if (langsFull !== "авыловаыловаылоаывллоавы") {
              let langsText = langsFull
                .map((lang: { id: any; native_name: any }) => `<a href="/search/people?c[name]=0&c[lang]=${lang.id}" class="vkuiLink Link-module__link--V7bkY ProfileModalInfoLink vkuiTappable vkuiInternalTappable vkuiTappable--hasActive vkui-focus-visible">${escapeHtml(lang.native_name)}</a>`)
                .join(", ");
          ...
          await addRelatives(userData, moreItemsLoaded, cachedRelativesInfo);
        }
        ...
        if (career && career.length > 0) {
          ...
          let jobPromises = career
            .slice()
            .reverse()
            .map(async (job: { group_id: string; company: string | null; city_name: string | null; city_id: any; from: any; until: any; position: string }) => {
              ...
              if (job.from && job.until) {
                ...
                let isArrJobFrom = getLang?.("profile_places_year_from");
                ...
                let isArrJobTo = getLang?.("profile_places_year_to");
        ...
        let profileMoreLink = profileMoreInfo.querySelector(".profile_more_info_link");
