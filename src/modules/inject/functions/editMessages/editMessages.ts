...
/*
 * Это максимальная тупая функция которую сделал Паша в бородатые времена
 * По хорошему её нужно переделать в модалку с отображением отредактированного сообщения
 * И подвязать сохранение на LP-ивент
 * Но это только если тебе не лень будет
 */

const editMessages = () => {
  document.arrive(".ConvoHistory__messageBlock", { existing: true }, function (e) {
    const key = "convoMessage" + getMessageProps(e as HTMLElement)[1] + "_" + getMessageProps(e as HTMLElement)[0];
    if (!localStorage.getItem(key)) {
      localStorage.setItem(key, e.innerHTML);
    }
    e.addEventListener("mouseover", function () {
      let buttonShowOrig = document.createElement("button");
      ...
      buttonShowOrig.classList.add("vkEnButtonShowOrig", "MessageActionsButton");
